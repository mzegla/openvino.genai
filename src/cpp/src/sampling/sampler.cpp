...
void register_new_token(const Token& sampled_token,
    ...
    if (is_extend_sequence) {
        ...
        if (cb_verbose()) std::cout << "  -> Token appended! New len=" << running_sequence->get_generated_len() << "\n";
    } else {
        if (cb_verbose()) std::cout << "  -> Token NOT appended (is_extend_sequence=false)\n";
    }
    if (!is_validation_mode_enabled &&
        ...
        if (cb_verbose()) std::cout << "  -> Generation PAUSED due to low confidence\n";
    }
};
