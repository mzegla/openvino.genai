...
public:
    ...
    void allocate_cache_if_needed(size_t num_kv_blocks) {
        ...
        if (m_num_allocated_kv_blocks >= num_kv_blocks) {
            if (cb_verbose()) std::cout << "  -> Cache already sufficient, returning early\n";
            return;
        }
        if (cb_verbose()) std::cout << "  -> Proceeding with allocation...\n";
        try {
            ...
            if (m_context) {// Allocate KV caches
                if (cb_verbose()) std::cout << "  Using RemoteContext (GPU) path\n";
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_num_decoder_layers; ++decoder_layer_id) {
                    ...
                    if (cb_verbose()) std::cout << "    Layer " << decoder_layer_id << ": key=" << key_cache_shape 
                ...
                if (cb_verbose()) std::cout << "  Using CPU (non-RemoteContext) path\n";
                for (size_t decoder_layer_id = 0; decoder_layer_id < m_num_decoder_layers; ++decoder_layer_id) {
                    ov::Shape value_cache_shape = set_kv_blocks(m_value_shapes[decoder_layer_id], num_kv_blocks);
                    ov::Shape key_cache_shape = set_kv_blocks(m_key_shapes[decoder_layer_id], num_kv_blocks);
                    if (cb_verbose()) std::cout << "    Layer " << decoder_layer_id << ": key=" << key_cache_shape 
            ...
            if (cb_verbose()) std::cout << "  Allocation complete! m_num_allocated_kv_blocks now = " << m_num_allocated_kv_blocks << "\n";
        }
